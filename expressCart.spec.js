...
context("Cypress for em", () => {
  ...
  for (let i=0; i<repeatTimes; i++) {
      ...
      it("can pay for product with existed account", () => {
        let productId = getRandomInt(1, 6);
        if (productId == 2) productId += 1;
        cy.get(
          ":nth-child(" +
            productId +
            ") > .thumbnail > .product_wrapper > a > .product-title"
        ).click();
        // cy.get(':nth-child(3) > .btn').should('be.visible').click();
        // cy.get(':nth-child(3) > .btn').click()
        ...
        cy.get(
          ...
          .should("be.visible")
          .click();
        let userId = getRandomInt(0, 30);
        cy.get("#customerLoginEmail").type(userList[userId] + '@gmail.com');
        cy.get("#customerLoginPassword").type("12345");
        cy.get('#customerLogin').should('be.visible').click()
        cy.get('#stripeButton').should('be.visible').click()
      });
  }
